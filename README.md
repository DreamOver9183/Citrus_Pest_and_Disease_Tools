...
ShowResultsWeb/backend/          FastAPI 後端
  app/routers/                   API 路由（sessions / datasets / exports / devices / inference / metrics）
  app/services/                  業務邏輯（模型管理、資料集分析、匯出 job、裝置探測）
  app/utils/                     ZIP 安全處理、圖片裁切
  tests/                         pytest 單元測試
...
完整的模組職責、設計決策與各項功能的技術細節，請參考 [docs/architecture.md](docs/architecture.md)。
...
更完整的已知限制清單與各項限制的技術背景，請參考 [docs/architecture.md](docs/architecture.md#7-已知限制)。
