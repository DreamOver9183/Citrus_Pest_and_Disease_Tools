...
from app.schemas import ReportGenerateRequest, ReportPayload, ReportsPayload
...
@router.post("/reports", response_model=ApiResponse[ReportPayload])
def create_report(payload: ReportGenerateRequest):
    if not payload.job_ids:
        raise ApiException("validation_error", "請至少選擇一筆評估結果")
    ...
    if not jobs:
        raise ApiException("precondition_failed", "選取的評估結果都不存在或尚未完成")
    ...
    try:
        meta = report_service.generate_report(jobs, plot_paths, dataset_stats, payload.title)
    except Exception as exc:  # noqa: BLE001
        raise ApiException("internal_error", f"報告產生失敗：{exc}")

    return ok({"report": meta, "message": None})


@router.get("/reports", response_model=ApiResponse[ReportsPayload])
def list_reports():
    return ok({"reports": report_service.list_reports()})
...
def download_report(report_id: str):
    ...
    if path is None:
        raise ApiException("not_found", "找不到指定的報告")
...
@router.delete("/reports/{report_id}", response_model=ApiResponse[ReportPayload])
def delete_report(report_id: str):
    if not report_service.delete_report(report_id):
        raise ApiException("not_found", "找不到指定的報告")
    return ok({"report": None, "message": "已刪除"})
