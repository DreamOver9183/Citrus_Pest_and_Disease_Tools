...
@router.get("/evaluations/targets", response_model=EvalTargetsResponse, response_model_exclude_unset=True)
def list_targets():
    """
    列出可用的模型與資料集。
    ...
    for stats in datasets_snapshot:
        ...
        datasets.append({
            ...
            "available": ok,
            "reason": reason,
            "splits": available_splits(stats) if ok else [],
            "default_split": preferred_split(stats) if ok else None,
        })
...
@router.post("/evaluations", response_model=Union[EvalJobResponse, ErrorResponse], response_model_exclude_unset=True)
def submit_evaluation(session_id: str = Form(...), dataset_id: str = Form(...), split: str = Form(None)):
    with SESSIONS_LOCK:
        session = ACTIVE_SESSIONS.get(session_id)
        session = dict(session) if session else None
    if session is None:
        return {"status": "error", "message": "找不到指定的模型 session"}
    if session.get("model_arch") != "yolo":
        return {"status": "error", "message": "目前只支援 YOLO 架構的評估。"}

    with DATASETS_LOCK:
        dataset = ACTIVE_DATASETS.get(dataset_id)
        dataset = dict(dataset) if dataset else None
    if dataset is None:
        return {"status": "error", "message": "找不到指定的資料集"}

    ok, reason = describe_availability(dataset)
    if not ok:
        return {"status": "error", "message": reason}

    chosen = split or preferred_split(dataset)
    if chosen not in available_splits(dataset):
        return {
            "status": "error",
            "message": f"這個資料集沒有名為「{chosen}」的 split，可用的有：{'、'.join(available_splits(dataset))}",
        }

    try:
        job = evaluation_service.submit_evaluation(session, dataset, chosen)
    except queue.Full:
        return {"status": "error", "message": "評估佇列已滿，請等待進行中的評估完成後再試"}

    return {"status": "success", "job": job, "message": None}


@router.get("/evaluations", response_model=EvalJobsResponse, response_model_exclude_unset=True)
def list_evaluations():
    return {"status": "success", **evaluation_service.get_jobs_snapshot()}


@router.get("/evaluations/{job_id}", response_model=Union[EvalJobResponse, ErrorResponse], response_model_exclude_unset=True)
def get_evaluation(job_id: str):
    job = evaluation_service.get_job(job_id)
    if job is None:
        return {"status": "error", "message": "找不到指定的評估紀錄"}
    return {"status": "success", "job": job, "message": None}
...
def get_evaluation_plot(job_id: str, key: str):
    ...
    if path is None:
        return {"status": "error", "message": "找不到指定的圖表"}
    return FileResponse(path, media_type="image/png")


@router.post("/evaluations/{job_id}/delete", response_model=Union[EvalJobResponse, ErrorResponse], response_model_exclude_unset=True)
def delete_evaluation(job_id: str):
    if not evaluation_service.delete_job(job_id):
        return {"status": "error", "message": "找不到指定的評估紀錄"}
    return {"status": "success", "job": None, "message": "已刪除"}
